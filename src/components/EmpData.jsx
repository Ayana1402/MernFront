...
function deleteemp(id){
  axiosInstance.delete('https://mernback-f3c7.onrender.com/api/delete/'+id).then((res)=>{
    console.log('https://mernback-f3c7.onrender.com/api/delete/'+id)
    alert('Employee details deleted succesfully')
    window.location.reload(false);
  })
}
// function editemp(){
//   axiosInstance.put('http://localhost:4000/api/update').then((res)=>{
//    navigate("/emplist");
//     window.location.reload(false);
//   })
// }
        ...
        <TableBody style={{backgroundColor:'#fffde7'}}>
          {data.map((row,i) => (
            <TableRow key={i}>
              ...
              <TableCell align="center"><Button onClick={()=>{updateemp(row)}}>Edit</Button></TableCell>
              <TableCell align="left"><Button onClick={()=>{deleteemp(row._id)}}>Delete</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
